...
const stripePromise = loadStripe(import.meta.env.VITE_REACT_STRIPE_KEY || "pk_test_51T0Guz5SOSBGyDvnnJVy75ReM2WBteIQisI1J5BXtWW9WaXBu0BVGGTo3oplTlceGsxNorYY4EyTA5bxGbBOKN8h00tSy5tEtd");

// Payment Form Component
const PaymentForm = ({ order, onSuccess, onError, onClose, amount }) => {
    ...
    useEffect(() => {
        const createPaymentIntent = async () => {
            try {
                ...
                // Validate amount
                const paymentAmount = order.total || order.totalAmount || 0;
                if (paymentAmount <= 0) {
                ...
                const response = await parseAxios.post(
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (paymentIntent && paymentIntent.status === "succeeded") {
                ...
                try {
                    const verifyResponse = await parseAxios.post(
...
if (result?.success) {
  ...
  throw new Error(result?.message || "Payment verification failed");
                    ...
                    throw new Error("Payment succeeded but verification failed. Please contact support.");
                }
            } else {
